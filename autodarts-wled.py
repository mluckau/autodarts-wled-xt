...
def parse_effects_argument(effects_argument, custom_duration_possible=True):
    if (
        effects_argument == None
        or effects_argument == ["x"]
        or effects_argument == ["X"]
    ):
    ...
    for effect in effects_argument:
        try:
            ...
            if effect_declaration == "ps":
                ...
                if (
                    custom_duration_possible == True
                    and len(effect_params) >= 3
                    and effect_params[2].isdigit() == True
                ):
...
def parse_score_area_effects_argument(score_area_effects_arguments):
    ...
    if len(area) == 2 and area[0].isdigit() and area[1].isdigit():
        return (
            ...
            parse_effects_argument(score_area_effects_arguments[1:]),
        )
    else:
...
def process_variant_x01(msg):
    if msg["event"] == "darts-thrown":
        ...
        if (
            HIGH_FINISH_ON != None
            and int(msg["game"]["dartsThrownValue"]) >= HIGH_FINISH_ON
            and HIGH_FINISH_EFFECTS != None
        ):
...
def connect_data_feeder():
    def process(*args):
        ...
        WS_DATA_FEEDER = websocket.WebSocketApp(
            data_feeder_host,
            ...
            if lastMessage != m:
                ...
                if "state" in m and waitingForIdle == True:
                    ...
                    if "ps" in ide and ide["ps"] == str(m["state"]["ps"]):
                        ...
                        if (
                            "col" in ide["seg"]
                            and ide["seg"]["col"][0] not in seg["col"]
                        ):
                            is_idle = False
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        required=False,
        help="Connection to data feeder",
    )
    ap.add_argument(
        "-WEPS",
        "--wled_endpoints",
    ...
    ap.add_argument(
        ...
        default=0,
        required=False,
        help="Duration of a played effect in seconds. After that WLED returns to idle. 0 means infinity duration.",
    )
    ap.add_argument(
        "-BSS",
        "--board_stop_start",
        ...
        type=float,
        required=False,
        help="If greater than 0.0 stops the board before playing effect",
    )
    ap.add_argument(
        "-BRI",
        "--effect_brightness",
        type=int,
        choices=range(1, 256),
        default=DEFAULT_EFFECT_BRIGHTNESS,
        required=False,
        help="Brightness of current effect",
    )
    ap.add_argument(
        "-HFO",
        "--high_finish_on",
        ...
        default=None,
        required=False,
        help="Individual score for highfinish",
    )
    ap.add_argument(
        "-HF",
        "--high_finish_effects",
        ...
        required=False,
        nargs="*",
        help="WLED effect-definition when high-finish occurs",
    )
    ap.add_argument(
        "-IDE",
        "--idle_effect",
        default=[DEFAULT_EFFECT_IDLE],
        required=False,
        nargs="*",
        help="WLED effect-definition when waiting for throw",
    )
    ap.add_argument(
        "-G",
        "--game_won_effects",
        ...
        required=False,
        nargs="*",
        help="WLED effect-definition when game won occurs",
    )
    ap.add_argument(
        "-M",
        "--match_won_effects",
        ...
        required=False,
        nargs="*",
        help="WLED effect-definition when match won occurs",
    )
    ap.add_argument(
        "-B",
        "--busted_effects",
    ...
    for v in range(0, 181):
        ...
        ap.add_argument(
            ...
            required=False,
            nargs="*",
            help="WLED effect-definition for score " + val,
        )
    for a in range(1, 13):
        area = str(a)
        ap.add_argument(
            "-A" + area,
            "--score_area_" + area + "_effects",
            default=None,
            required=False,
            nargs="*",
            help="WLED effect-definition for score-area",
        )
    ...
    try:
        ...
        WLED_EFFECTS = requests.get(
            effect_list_url, headers={"Accept": "application/json"}
        )
        WLED_EFFECTS = [we.lower().split("@", 1)[0] for we in WLED_EFFECTS.json()]
        WLED_EFFECT_ID_LIST = list(range(0, len(WLED_EFFECTS) + 1))
        ppi(
            "Your primary WLED-Endpoint ("
            + effect_list_url
            + ") offers "
            + str(len(WLED_EFFECTS))
            + " effects"
        )
    except Exception as e:
    ...
    for a in range(1, 13):
        parsed_score_area = parse_score_area_effects_argument(
            args["score_area_" + str(a) + "_effects"]
        )
